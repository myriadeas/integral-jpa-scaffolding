...
function ${entity.name}EditCtrl($scope, $dialog, $log, $routeParams, $location, Restangular, Notification, DomainUtils, ErrorBinder, SelectionLookup, Lookup, ConfirmDeleteDialog, Localization) {
...
#foreach($attribute in $entity.attributes)
	...
	${entity.name.toLowerCase()}Rest.get().then(function(response){
       ...
       $scope.generalInfo = DomainUtils.getGeneralInfo(${attribute.javaType.simpleName.toLowerCase()});
	}, function(errorResponse) {
	...
	$scope.$on("event:ModalSelectedItems", function(event, response){
		response.parentDomainName = "${entity.name.toLowerCase()}";
		$log.log("model selected response=", response);
		DomainUtils.updateModel($scope, response);
	});
	
}

function ${entity.name}ViewCtrl($dialog, $scope, $routeParams, $location, Restangular, Notification, ErrorBinder, SelectionLookup, Lookup, ConfirmDeleteDialog, DomainUtils, Localization) {
	var ${entity.name.toLowerCase()}Rest = Restangular.one('${entity.name.toLowerCase()}', $routeParams.id);
	var selectionLookups = {
		"rest" : ${entity.name.toLowerCase()}Rest,
		"name" : "${entity.name.toLowerCase()}",
...
function ${entity.name}CreateCtrl($scope, $routeParams, $location, $log, Restangular, Notification, DomainUtils, ErrorBinder, SelectionLookup, Localization) {
